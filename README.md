...
such a comparison mentioned in Vanhasbroeck et al. (2024). The code
within this project combines the structure of an R package alongside
scripts that call upon the functions used within that package, providing
its own ecosystem in which functions evolve together with our research
purposes. While in-principle the `discounting` package can be used by
others, it is not necessarily meant to and may, therefore, feel
user-unfriendly.

## Installation

To install the package and use its functions, one can use the `remotes`
package:

    remotes::intall_github("ndpvh/discounting-affect")

One can then call the `library()` function to load the package’s
functions:
...
Most of the package’s functionality depends on the definition of an
instance of a model, namely of the exponential discounting model
([`exponential()`](https://ndpvh.github.io/discounting-affect/reference/exponential.html);
Samuelson, 1937, Rutledge et al., 2014), the quasi-hyperbolic
discounting model
([`quasi_hyperbolic()`](https://ndpvh.github.io/discounting-affect/reference/quasi_hyperbolic.html);
Laibson, 1997), or the double-exponential discounting model
([`double_exponential()`](https://ndpvh.github.io/discounting-affect/reference/double_exponential.html);
van den Bos & McClure, 2013). For example, we can create a
...
Simulating from this model can be achieved through the
...
transform our dataset to an instance of the
[`dataset`](https://ndpvh.github.io/discounting-affect/reference/dataset.html)
class and can then call the function
...
One can consult the package’s vignettes for more information on the
project, the
...
Samuelson, P. A. (1937). A note on measurement of utility. *The Review
...
<a href = "https://doi.org/10.1073/pnas.1407535111">10.1073/pnas.1407535111</a>

van den Bos, W. & McClure, S. M. (2013). Towards a general model of
temporal discounting. *Journal of the Experimental Analysis of Behavior,
99*, 58–73. doi:
<a href = "https://doi.org/10.1002/jeab.6">10.1002/jeab.6</a>
...
<a href = "https://doi.org/10.1016/j.jmp.2024.102856">10.1016/j.jmp.2024.102856</a>
